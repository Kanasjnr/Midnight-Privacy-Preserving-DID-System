...
export class CredentialIssuer {
  ...
  /**
   * Creates a "cracked" credential commitment (Merkle root).
   * Each attribute is hashed with a unique salt to ensure zero-knowledge.
   */
  public createCredentialCommitment(data: CredentialData): { root: string; salts: Record<string, string> } {
    ...
    // Simplified Merkle root (just hashing all leaves together for this tutorial)
   ...
   */
  public createIssuanceEntry(holderDidCommitment: string, schemaId: string, credentialHash: string): string {
  ...
  /**
   * Signs a credential (simplified for the Midnight context).
